...
class Addrlts(object):
    ...
    def get_sheng(self,s):
        rlt = re.search(("(.*?省)|(.*?自治区)|上海市|北京市|天津市|重庆市"),s)

        if rlt == None or len(rlt.group(0))>5:
            if s[0:3] == "黑龙江":
                return "黑龙江"
            else:
                return s[0:2]
    ...
    def get_shi(self,s):
        rlt = re.search("(.*?自治州)|(.*?[市])", s)

        if rlt == None or len(rlt.group(0))>7:
            for i in cities:
                if s[0:2] in i:
                    return i
            return ""
    ...
    def get_xz(self,s):
        rlt = re.search("(.*?街道)|(.*?[镇乡])", s)
        if rlt == None:
            return ""
        return rlt.group(0)

    def get_jiedao(self,s):
        rlt = re.search("(.*?[路街巷道])", s)
        if rlt == None:
            return ""
        return rlt.group(0)
...
phone_num = addrlts.get_phone_num(ns)
ns = ns.replace(phone_num, "", 1)
ns = ns.replace(".", "", 1)

sheng = addrlts.get_sheng(ns)
if sheng == "上海" or sheng == "北京" or sheng == "天津" or sheng == "重庆":
    sheng = sheng
    ns = ns.replace(sheng,sheng+"市",1)
elif sheng == "上海市" or sheng == "北京市" or sheng == "天津市" or sheng == "重庆市":
    sheng = sheng[0:2]
elif sheng[-1] != "省" and sheng[-1] != "区":
    ns = ns.replace(sheng,"",1)
    sheng = sheng + "省"
else:
    ns = ns.replace(sheng,"",1)

shi = addrlts.get_shi(ns)

if shi != "":
    ns = ns.replace(shi, "", 1)
    if (shi[-1] != "市") and (shi[-3:-1] != "自治"):
        shi = shi+"市"

xian = addrlts.get_xian(ns)
if xian != "":
    l = len(xian)
    if(xian[-1]==ns[l-1]):
        ns = ns.replace(xian, "", 1)
    else:
        ns = ns.replace(xian[:-1], "", 1)
